{{-- resources/views/product_master/list.blade.php --}}
@extends('layouts.master') {{-- your master file --}}

@section('title', 'Max POS Product List')
...
<!-- Floating create button -->
<a href="{{ route('productcreate') }}" class="fab"><i class="fas fa-plus"></i></a>
@endsection
...
$(document).ready(function(){
    ...
    let typingTimer;
    const typingDelay = 300; // debounce delay in ms

    function fetchProducts(query='', url="{{ route('items.list') }}"){
        $.get(url, { q: query }, function(data){
            // Update grid
            $grid.html(data.rows);
            // Update pagination
            $pagination.html(data.pagination);
            // Update autocomplete (top 5)
            $list.html('');
