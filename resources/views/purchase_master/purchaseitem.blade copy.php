...
/* HEADER */
.header-row { display:flex; flex-wrap:wrap; align-items:center; margin-bottom:15px; }
.header-logo { flex:0 0 150px; text-align:center; margin-bottom:10px; }
...
#purchaseNav label { color: #fff !important; font-weight:500; }

/* TABLES */
.table-container { overflow-x:auto; }
.cart-table { width:100%; border-collapse:collapse; min-width:900px; }
.cart-table th, .cart-table td { padding:6px 8px; text-align:center; border-bottom:1px solid #ddd; }
...
.cart-table tbody tr:hover { background-color:#e3f2fd; cursor:pointer; }
...
<div id="tabletWrapper" class="tablet-container">
    ...
    <div class="header-row valign-wrapper">
        <div class="header-logo">
            <img src="https://via.placeholder.com/120x50?text=MaxPos" alt="MaxPos Logo">
        </div>
            ...
            <div class="col s12 m8">
                <div class="row">
                    <div class="input-field col s8">
                        <input type="text" id="productCodeInput" placeholder="Enter product code" autofocus>
...
<div id="editModal" class="modal">
  <div class="modal-content">
    ...
    <div class="row" style="align-items:center; gap:10px;">
      <div class="col s6" style="display:flex; align-items:center; gap:10px;">
        <label>Unit:</label>
...
document.getElementById('discountField').addEventListener('input',calculateTotals);
document.getElementById('taxField').addEventListener('input',calculateTotals);
document.getElementById('paidField').addEventListener('input',calculateTotals);

function showToast(message,color='green'){ M.toast({html:`<span>${message}</span>`, classes:`${color} darken-1 tablet-toast`, displayLength:2500}); }
...
function addToCart(pid,vid){
    ...
    if(existing){
        Swal.fire({
            title:'Already Added!',
            text:'Do you want to add 1 more quantity?',
            icon:'question',
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    products.forEach(p=>p.variants.forEach(v=>{
        tbody.insertAdjacentHTML('beforeend',`
            <tr onclick="addToCart(${p.id},${v.id}); M.Modal.getInstance(document.getElementById('productSearchModal')).close();">
...
function openEditModal(i){
    ...
    const selectedVariant = product.variants.find(v=>v.id===item.variant_id);
    document.getElementById('modalPriceDisplay').innerText = selectedVariant.last_purchase_price;
...
document.getElementById('modalUnitSelect').addEventListener('change', ()=>{
    const variantId = Number(document.getElementById('modalUnitSelect').value);
    const product = products.find(p=>p.id===cart[editingIndex].product_id);
