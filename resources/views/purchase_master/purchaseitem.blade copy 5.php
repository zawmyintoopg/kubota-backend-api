                ...
                <div class="card summary-card">
                    <div class="card-content">
                        ...
                        <div class="row">
                            ...
                            <div class="col s12">
                                <a class="btn green" id="savePrintBtn"><i class="fas fa-print"></i> Save & Print</a>
...
function addToCart(pid,vid){
    ...
    if(existing){ existing.qty +=1; render(); showToast('Added 1 more'); return; }
    ...
    render(); showToast('Product added','green');
}

// ===== SAVE & PRINT =====
document.getElementById('savePrintBtn').addEventListener('click',()=>{
    if(cart.length===0){ Swal.fire('Empty Cart','Add products first','error'); return; }
    const paid = Number(document.getElementById('paidField').value||0);
    const grand = Number(document.getElementById('grand_total_input').value||0);
    if(paid<grand){ Swal.fire('Invalid Paid','Check paid amount','error'); return; }

    // Render print
    const printArea=document.getElementById('printArea');
    const shopName = 'Max POS Shop';
    let html=`<h2>${shopName}</h2>`;
    html += `<p>Date: ${document.getElementById('purchaseDate').value}</p>`;
    html += `<table><thead>
    <tr><th>#</th><th>Code</th><th>Product</th><th>Unit</th><th>Qty</th><th>Buy</th><th>Discount</th><th>Amount</th></tr>
    </thead><tbody>`;
    cart.forEach((c,i)=>{
        const total = c.qty*c.buyprice - (c.discount||0);
        html += `<tr>
    ...
    html += `</tbody></table>`;
    html += `<p>Sub Total: ${document.getElementById('subTotal').innerText}</p>`;
    html += `<p>Discount: ${document.getElementById('discountField').value}</p>`;
    html += `<p>Tax: ${document.getElementById('taxField').value}</p>`;
    html += `<p>Grand Total: ${document.getElementById('grandTotal').innerText}</p>`;
    html += `<p>Paid: ${document.getElementById('paidField').value}</p>`;
    html += `<p>Balance: ${document.getElementById('balance').innerText}</p>`;
    printArea.innerHTML = html;
    window.print();
    cart=[]; render(); showToast('Purchase saved & printed','green');
});

// ===== CANCEL =====
document.getElementById('cancelBtn').addEventListener('click',()=>{ cart=[]; render(); showToast('Purchase cancelled','red'); });

// ===== OTHER FEATURES (Autocomplete, Edit Modal, Tablet Mode) =====
