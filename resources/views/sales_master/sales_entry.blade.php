...
/* NAVBAR */
.nav-desktop{display:flex;justify-content:space-between;align-items:center;padding:10px;background:#0d6efd;color:#fff;}
...
<div class="nav-desktop">
    <a class="btn text-white btn-sm btn-outline-white" href="{{ route('salePage') }}">
      <i class="fas fa-arrow-left"></i> Back
    </a>
    <div><h5>Create Sales</h5></div>
    <div>
...
<button class="btn btn-primary btn-lg w-100" id="save-sale">
...
<h5 class="mb-0 fw-bold">Sale Summary</h5>
<div style="width:32px"></div>
</div>

<form id="sale-form" action="{{ route('sales.save') }}" method="POST">
@csrf
...
// ---- CART UPDATE ----
function updateCart(){
    let t=0; $('#cart-items').empty();
    cart.forEach((i,idx)=>{
        t+=i.price*i.qty;
        $('#cart-items').append(`<div class="d-flex justify-content-between border p-1">${i.name} x ${i.qty} <button class="btn btn-sm btn-danger remove" data-i="${idx}">x</button></div>`);
    });
    $('#cart-subtotal,#mobile-cart-subtotal').text(t);
...
$(document).on('click','.customer-item',function(){
    ...
    let cid=$(this).data('id'); $('#customer_id').val(cid);
...
$('#sal-form').submit(function (e) {
    e.preventDefault();
    if (!$('#customer_id').val()) {Swal.fire({ icon:'warning', title:'Please select a customer', timer:1200, showConfirmButton:false }); return;}
