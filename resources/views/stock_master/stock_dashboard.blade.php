        ...
        <table class="table table-bordered table-striped">
            ...
            <tbody>
                ...
                <tr @if($stock->onhand_qty <= $stock->orderlevel_qty) style="background-color: #f8d7da;" @endif>
                    <td>{{ $stock->variant?->product_id ?? 'N/A' }}</td>
                    <td>{{ $stock->orderlevel_qty }}</td>
                    <td>{{ $stock->onhand_qty }}</td>
                    <td>{{ $stock->converted_quantity ?? 0 }}</td>
                    <td>${{ number_format($stock->total_value ?? 0,2) }}</td>
                    <td>
...
function renderCharts(totalValue, lowStockCount, totalStocks, lowStockProducts){
    new Chart(document.getElementById('totalValueChart'), {
        type: 'doughnut',
        data: { labels: ['Total Value'], datasets: [{ data:[totalValue], backgroundColor:['#4e73df'], hoverOffset:10 }]},
        options:{
            responsive:true,
            plugins:{ legend:{ display:false }, tooltip:{ callbacks:{ label:()=> 'Total Stock Value: $'+totalValue.toLocaleString() }}}
        }
    });

    new Chart(document.getElementById('lowStockChart'), {
        type: 'doughnut',
        data: { labels:['Low Stock','OK'], datasets:[{ data:[lowStockCount,totalStocks-lowStockCount], backgroundColor:['#e74a3b','#1cc88a'], hoverOffset:10 }]},
        options:{
            responsive:true,
            plugins:{
                legend:{ position:'bottom' },
                tooltip:{ callbacks:{ label:function(c){ return c.label==='Low Stock'? 'Low Stock ('+lowStockCount+'): '+lowStockProducts.join(',') : 'OK Stock ('+(totalStocks-lowStockCount)+')'; }}}
            },
